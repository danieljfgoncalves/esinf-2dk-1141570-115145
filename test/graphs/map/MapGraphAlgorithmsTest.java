...
public class MapGraphAlgorithmsTest {
    ...
    MapGraph<String, String> completeMap = new MapGraph<>(false);

    public MapGraphAlgorithmsTest() {
    }
...
//        completeMap.insertEdge("Lisboa","Faro","A2",280);
    ...
    public void testBreadthFirstSearch() {
        ...
        assertTrue("Should be null if vertex does not exist", MapGraphAlgorithms.BreadthFirstSearch(completeMap, "LX") == null);

        LinkedList<String> path = MapGraphAlgorithms.BreadthFirstSearch(completeMap, "Faro");
        assertTrue("Should be just one", path.size() == 1);

        Iterator<String> it = path.iterator();
        assertTrue("it should be Faro", it.next().compareTo("Faro") == 0);

        path = MapGraphAlgorithms.BreadthFirstSearch(completeMap, "Porto");
        assertTrue("Should give seven vertices ", path.size() == 7);

        path = MapGraphAlgorithms.BreadthFirstSearch(completeMap, "Viseu");
    ...
    public void testDepthFirstSearch() {
        ...
        assertTrue("Should be null if vertex does not exist", MapGraphAlgorithms.DepthFirstSearch(completeMap, "LX") == null);

        path = MapGraphAlgorithms.DepthFirstSearch(completeMap, "Faro");
        assertTrue("Should be just one", path.size() == 1);

        Iterator<String> it = path.iterator();
        assertTrue("it should be Faro", it.next().compareTo("Faro") == 0);

        path = MapGraphAlgorithms.DepthFirstSearch(completeMap, "Porto");
        assertTrue("Should give seven vertices ", path.size() == 7);

        path = MapGraphAlgorithms.DepthFirstSearch(completeMap, "Viseu");
        assertTrue("Should give 3 vertices", path.size() == 3);
